...
function readCache() {
  try {
    ...
    if (typeof j.fetched === 'number' && Date.now() - j.fetched < ttl && Array.isArray(j.ids)) {
...
async function fetchBoardCardIds(boardId, key, token) {
  ...
  for (;;) {
    const u = new URL(`https://api.trello.com/1/boards/${encodeURIComponent(boardId)}/cards`)
    ...
    if (!res.ok) {
      throw new Error(`Trello HTTP ${res.status} for board ${boardId}: ${body.slice(0, 200)}`)
    }
...
async function main() {
  ...
  if (process.env.SKIP_TRELLO_VALIDATE === '1' || process.env.SKIP_TRELLO === '1') {
    process.exit(0)
  }
  ...
  if (!commitFile) {
    console.error('Usage: validate-commit-trello.mjs <path-to-commit-message-file>')
    process.exit(1)
  }
  ...
  if (!FORMAT.test(first)) {
    ...
    console.error('(7–8 alphanumeric id from the Trello card URL, e.g. AbCdEfGh - add login page)')
    process.exit(1)
  }

  if (!process.env.TRELLO_API_KEY || !process.env.TRELLO_TOKEN || !process.env.TRELLO_BOARD_ID) {
    console.error(
      'Missing Trello env. Set TRELLO_API_KEY, TRELLO_TOKEN, TRELLO_BOARD_ID (see .env.example).'
    )
    process.exit(1)
  ...
  if (!valid.has(id)) {
    console.error(`Unknown Trello card id "${id}". It is not on the configured board(s).`)
    console.error('Refresh cache: delete .husky/.trello-cache.json and commit again, or add the card in Trello.')
    process.exit(1)
  }
}
